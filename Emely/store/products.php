...
$query = $link->query("SELECT ID, productName, imgThumb FROM products");
if ($query) {
  $count = 0;
  while ($row = $query->fetch_assoc()) {
    if ($count%3 == 0) {
    ...
    echo '<a href="showProduct.php?ID='.$row["ID"].'""> <img class="img-rounded" data-src="holder.js/140x140"></a>';
    echo '<p alt="showProduct.php?ID='.$row["ID"].'">' . $row["productName"] . '</p></div>';
    if ($count%3 == 2){
      echo '</div>';
    }
    $count = $count + 1;
